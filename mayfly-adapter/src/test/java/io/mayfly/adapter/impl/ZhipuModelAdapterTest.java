...
@DisplayName("ZhipuModelAdapter 单元测试")
class ZhipuModelAdapterTest {
    
    private final ZhipuModelAdapter adapter = new ZhipuModelAdapter();
    ...
    class CreateChatModelTests {
        ...
        void testCreateChatModelWithFullConfig() {
            ModelConfig config = ModelConfig.builder()
                ...
                .model("glm-4-plus")
                .apiKey("test-api-key")
                .baseUrl("https://custom.zhipu.api")
                .weight(80)
                .timeout(60000)
                .build();
        ...
        void testCreateChatModelWithCustomBaseUrl() {
            ModelConfig config = ModelConfig.builder()
                .name("zhipu-custom-url")
                .provider("zhipu")
                .model("glm-4")
                .apiKey("test-api-key")
                .baseUrl("https://private.bigmodel.cn/api/paas/v4")
                .build();
    ...
    class MockHttpCallTests {
        ...
        private ZhipuModelAdapter createMockAdapter(HttpClient mockClient) {
            return new ZhipuModelAdapter() {
                @Override
        ...
        void testSuccessfulCallReturnsContent() {
            HttpClient mockClient = (url, headers, requestBody) -> {
                return Map.of(
                    ...
                    "choices", List.of(Map.of(
                        "index", 0,
                        "message", Map.of("role", "assistant", "content", "Hello from Zhipu!"),
                        "finish_reason", "stop"
                    ))
                );
            };
            
            ZhipuModelAdapter mockAdapter = createMockAdapter(mockClient);
            ZhipuModelAdapter.ZhipuChatModel chatModel = (ZhipuModelAdapter.ZhipuChatModel) 
            ...
            assertEquals("Hello from Zhipu!", response.getResult().getOutput().getText());
        }
        ...
        void testMultiTurnMessageConversion() {
            HttpClient mockClient = (url, headers, requestBody) -> {
                // requestBody 是 ZhipuChatRequest 对象，需要通过反射获取属性
                assertEquals("glm-4", getPropertyValue(requestBody, "model"));
        ...
        @Test
        @DisplayName("测试AssistantMessage消息转换")
        void testAssistantMessageConversion() {
            HttpClient mockClient = (url, headers, requestBody) -> {
                ...
                return Map.of(
                    ...
                    "choices", List.of(Map.of(
                        "index", 0,
                        "message", Map.of("role", "assistant", "content", "Response with history"),
                        "finish_reason", "stop"
                    ))
                );
            };
            ...
            org.springframework.ai.chat.prompt.Prompt prompt = 
                new org.springframework.ai.chat.prompt.Prompt(List.of(
                    ...
                    new org.springframework.ai.chat.messages.UserMessage("How are you?")
                ));
            
            org.springframework.ai.chat.model.ChatResponse response = chatModel.call(prompt);
            
            assertNotNull(response);
            assertEquals("Response with history", response.getResult().getOutput().getText());
        }
    }
}
